...
static int pcscParseOneKey (pcscConfigT *config, json_object *keyJ, pcscKeyT *key)
{
    ...
    err= rp_jsonc_unpack (keyJ, "{ss,s?i,so !}"
        ,"uid", &key->uid
        ,"idx", &key->kidx
        ,"value", &valueJ
    );
...
static int pcscParseOneTrailer (pcscConfigT *config, json_object *trailerJ, pcscTrailerT **trailer)
{
    ...
    err= rp_jsonc_unpack (trailerJ, "{ss,ss,so !}"
        ,"keyA", &keyA
        ,"keyB", &keyB
        ,"acls", &valueJ
    );
...
static int pcscParseOneCmd (pcscConfigT *config, json_object *cmdJ, pcscCmdT *cmd)
{
    ...
    err= rp_jsonc_unpack (cmdJ, "{ss,ss,s?i,s?i,s?i,s?s,s?o,s?o,s?i !}"
        ,"uid", &cmd->uid
        ,"action", &cmdAction
        ,"sec", &cmd->sec
        ,"blk", &cmd->blk
...
pcscConfigT *pcscParseConfig (json_object *configJ, const int verbosity)
{
    ...
    err= rp_jsonc_unpack (configJ, "{s?s s?s ss s?i s?i s?i s?o s?o !}"
        , "uid", &config->uid
        , "info", &config->info
